...
class RunLogRepository(BaseRepository[RunLog]):
    ...
    async def get_by_status(self, status: str, *, skip: int = 0, limit: int = 100) -> List[RunLog]:
    ...
    async def get_recent(self, *, skip: int = 0, limit: int = 100) -> List[RunLog]:
        """Get recent run logs.
        ...
        Returns:
            List of run logs
        """
        statement = select(RunLog).order_by(RunLog.started_at.desc()).offset(skip).limit(limit)
        result = await self.session.execute(statement)
        return list(result.scalars().all())
