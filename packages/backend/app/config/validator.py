...
class ConfigValidator:
    ...
    def _validate_paths(self) -> None:
        ...
        for field, name in path_fields.items():
            ...
            # 检查路径是否存在
            if not path.exists():
                self.warnings.append(f"{name} '{path_str}' 不存在,将在首次使用时自动创建")
            else:
                # 检查是否可写
    ...
    def _validate_vendors(self) -> None:
        ...
        for field, name in vendor_fields.items():
            ...
            if value not in valid_vendors:
                self.warnings.append(
                    f"{name} 供应商 '{value}' 可能无效. 有效选项: {', '.join(valid_vendors)}"
                )
            else:
            ...
            if ":///" in db_url:
                ...
                if db_path and not db_path.startswith(":memory:"):
                    ...
                    if not db_dir.exists():
                        self.warnings.append(f"SQLite 数据库目录 '{db_dir}' 不存在,将自动创建")
    ...
    def _validate_api_keys(self) -> None:
        ...
        if provider in provider_key_map:
            ...
            if not os.getenv(env_key):
                self.warnings.append(f"LLM 提供商 '{provider}' 需要 {env_key} 环境变量")
            else:
                self.info.append(f"✓ {env_key} 已设置")

        # 检查数据供应商的 API 密钥
        if (
            self.settings.vendor_fundamental_data == "alpha_vantage"
            or self.settings.vendor_news_data == "alpha_vantage"
        ):
            if not os.getenv("ALPHA_VANTAGE_API_KEY"):
                self.warnings.append("使用 Alpha Vantage 但 ALPHA_VANTAGE_API_KEY 未设置")
            else:
    ...
    def _validate_numeric_ranges(self) -> None:
        ...
        if self.settings.max_debate_rounds < 0:
            self.errors.append(f"max_debate_rounds ({self.settings.max_debate_rounds}) 不能为负数")
        ...
        if self.settings.max_recur_limit < 1:
            self.errors.append(f"max_recur_limit ({self.settings.max_recur_limit}) 必须至少为 1")
