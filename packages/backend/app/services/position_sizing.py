...
class PositionSizingService:
    ...
    def calculate_quantity(
        ...
        if self.method == PositionSizingMethod.FIXED_DOLLAR:
            ...
            if stop_loss_price is None:
                logger.warning(
                    "Stop loss price not provided for risk-based sizing, using fixed percentage"
                )
                quantity = self._calculate_fixed_percentage(current_price, portfolio_value)
            else:
            ...
            if volatility is None:
                logger.warning(
                    "Volatility not provided for volatility-based sizing, using fixed percentage"
                )
                quantity = self._calculate_fixed_percentage(current_price, portfolio_value)
            else:
            ...
            if confidence_score is None:
                logger.warning(
                    "Confidence score not provided for Kelly criterion, using fixed percentage"
                )
                quantity = self._calculate_fixed_percentage(current_price, portfolio_value)
            else:
                quantity = self._calculate_kelly(current_price, portfolio_value, confidence_score)
        ...
        if quantity > max_quantity:
            logger.info(f"Quantity {quantity} exceeds max position size, capping at {max_quantity}")
    ...
    def _calculate_fixed_percentage(self, current_price: float, portfolio_value: float) -> float:
