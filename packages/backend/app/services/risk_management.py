...
from ..db.models import Position, RiskMetrics
    ...
    def _calculate_var(
        ...
        try:
            ...
            returns_matrix = np.array([
                historical_returns[s][:min_length] for s in symbols
            ]).T
            ...
            logger.info(
                f"Calculated VaR: 1d-95%=${abs(var_1day_95):,.0f}, "
                f"1d-99%=${abs(var_1day_99):,.0f}"
            )
